...
def generate_recommendations(df: pd.DataFrame, 
    ...
    pop_unnecessary_keys(final_users)

    return final_users

def pop_unnecessary_keys(profiles: list[dict[str, dict[str, any]]]):
    for profile in profiles:
        profile.pop('birth_date', None)
        profile.pop('birth_month', None)
        profile.pop('birth_year', None)
        profile.pop('gender', None)
        profile.pop('height', None)
        profile.pop('interested_in', None)
        profile.pop('middle_name', None)
        profile.pop('last_name', None)
        profile.pop('rating', None)
        profile.pop('relationship_status', None)
        profile.pop('sexual_orientation', None)
...
def run_mock_algorithm(username: str,
                       mock_data: list[dict[str]],
                       use_so_filter: bool):
    pass