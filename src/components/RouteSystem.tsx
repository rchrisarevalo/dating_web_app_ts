...
export const RoutingSystem = () => {
  const { auth, pending, error, username, profile_pic, status_code } =
    useFetchLogin();
  const profile_data = useFetchRoutes(
    "http://localhost:4000/get_user_routes",
    auth
  );
            ...
            !profile_data.error ? (
              ...
              >
                <ProtectedRoutes>
                  ...
                  <Route
                    path="/profile/options/update"
                    element={<Update username={username} />}
                  />
                  <Route
                    path="/profile/options/settings"
                    element={<AccountSettings username={username} />}
                  />
                  <Route
                    path="/profile/options/privacy"
                    element={<PrivacySettings username={username} />}
                  />
                  <Route
                    path="/profile/options/privacy/view_blocked_users"
                    element={<BlockedUsers />}
                  />
                  <Route
                    path="/profile/options/privacy/download_information"
                    element={<DownloadInfo />}
                  />
                  <Route
                    path="/profile/recent_messages"
                    element={<RecentMessages />}
                  />
                  <Route
                    path="/profile/follow_requests"
                    element={<ViewChatReqs />}
                  />
                  ...
                  {profile_data.chatRoutes.map((user: { username: string }) => (
                    <Route
                      ...
                      element={<MessageUser username={username} />}
                    />
                  ))}
                  <Route path="/tos" element={<TOS />} />
                  <Route
